...
const parseColorToRgb = (color: string): [number, number, number] => {
  if (color.startsWith('#')) {
    ...
    if (result) return [parseInt(result[1], 16) / 255, parseInt(result[2], 16) / 255, parseInt(result[3], 16) / 255];
  }
  if (color.includes('%')) {
    const parts = color.replace(/hsl\(|\)|deg|%/g, '').trim().split(/[\s,]+/);
    if (parts.length >= 3) {
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    u.uZoom.value           = zoom;
    u.uColor1.value         = new Float32Array(parseColorToRgb(color1));
    u.uColor2.value         = new Float32Array(parseColorToRgb(color2));
    u.uColor3.value         = new Float32Array(parseColorToRgb(color3));
  }, [
