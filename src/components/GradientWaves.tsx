...
float plasma(vec3 r, vec2 freq, vec4 tc) {
  float mx = r.x + tc.x;
  mx += uSwell * sin((r.y + mx) * 0.05 + tc.y);
  float my = r.y - tc.z;
  my += uTurbulence * cos(r.x * 0.0435 + tc.w);
...
void main() {
  ...
  vec2 freq = vec2(uWaveScale * 0.1428, (uWaveScale * uWaveRatio) * 0.3333);
  vec4 tc = vec4(T * 7.692, T * 1.234, T * 5.0, T * 1.408);
  float c, s;
  float vfov = 1.3659 / max(uZoom, 0.05);
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (enableMouseRef.current) {
        const tx = targetMouse[0];
        const ty = targetMouse[1];
