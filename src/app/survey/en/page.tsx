...
export default function SurveyPage() {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden font-outfit" style={{ background: "#070710" }}>
      ...
      <a href="/survey/ar"
        className="fixed top-5 right-5 z-50 flex items-center justify-center w-10 h-10 rounded-full border border-border bg-muted text-muted-foreground hover:text-foreground/80 hover:border-white/25 hover:bg-muted transition-all duration-200 backdrop-blur-sm"
        title="عربي"
        dir="rtl">
        ...
        <AnimatePresence mode="wait" initial={false}>
          ...
          {step === 0 && (
            <motion.div key="intro" {...FADE_UP} transition={DUR}>
              ...
              <h1 className="font-extrabold leading-[0.93] tracking-tight  mb-4" style={{ fontSize: "clamp(2.4rem,7vw,6.5rem)" }}>
                AI Writing vs<br />
              ...
              <p className="text-base md:text-lg text-white/50 max-w-xl mb-3 leading-relaxed font-light">
                <span className="font-semibold text-foreground/70">Perception Survey</span> — This survey aims to explore people&apos;s perceptions of Artificial Intelligence (AI) writing compared to human writing.
              </p>
              ...
              <div className="flex flex-wrap gap-2 mb-12">
                {["Tell Us About You", "1 · General Perception", "2 · Evaluation", "3 · Open-Ended"].map((s) => (
                  <span key={s} className="text-xs font-medium px-3 py-1 rounded-full border border-border text-muted-foreground">
                    {s}
                  </span>
                ))}
              </div>
          ...
          {step >= 1 && step <= TOTAL && q && (
            <motion.div key={`q${step}`} {...SLIDE} transition={DUR}>
              ...
              {/* Question */}
              <h2 className="font-extrabold leading-[0.95] tracking-tight  mb-4"
                style={{ fontSize: "clamp(2.4rem,5.5vw,5.5rem)", whiteSpace: "pre-line" }}>
                {q.label}
              </h2>
              {q.sub && <p className="text-sm md:text-base text-muted-foreground mb-10 font-light">{q.sub}</p>}
              ...
              {q.type === "textarea" && (
                <div className="max-w-2xl">
                  <textarea
                    ...
                    onChange={(e) => setAnswer(e.target.value)}
                    placeholder={q.placeholder}
                    className="w-full bg-transparent  text-lg md:text-xl font-light placeholder-white/20 resize-none outline-none border-b-2 pb-4 transition-[border-color] duration-300"
                    style={{ borderColor: (answer as string)?.trim() ? q.accent : "rgba(255,255,255,0.12)", caretColor: q.accent }}
                  />
                  {q.required && (
          ...
          {step === TOTAL + 1 && (
            <motion.div key="done" {...FADE_UP} transition={DUR}>
              ...
              <h2 className="font-extrabold leading-[0.93] tracking-tight  mb-6"
              ...
              <a href="../ar"
                className="inline-flex items-center gap-3 font-bold text-base px-8 py-4 rounded-2xl text-white/55 hover: border border-border hover:border-white/25 transition-[border-color,color,transform] duration-200 hover:scale-[1.03]">
