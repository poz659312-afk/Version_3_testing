...
* **قواعد التنسيق**:
  - الجداول: استخدمي الجداول القياسية بخط فاصل واحد فقط \`| العمود 1 | العمود 2 |\` (ممنوع منعاً باتاً استخدام الخطوط المزدوجة \`||\`).
  - علامة الدولار \`$\`: ممنوع وضع علامة \`$\` قبل أو بعد النصوص والأرقام العادية (مثل 1$ أو 2$)؛ تستخدم \`$\` فقط حصراً حول المعادلات الرياضية الإنجليزية مثل \`$x = 5$\`.
