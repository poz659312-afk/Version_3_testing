...
* **قواعد التنسيق الشكلي الصارمة (Clean Markdown Standards)**:
  ...
  - الجداول (Tables): كل صف في الجدول يجب أن يكون في سطر مستقل تماماً يبدأ بـ | وينتهي بـ |، مثل:
    | الرمز / البند | المعنى والوصف | الوحدة أو الملاحظات |
    | :--- | :--- | :--- |
    | G | ثابت الجاذبية العام | m^3 kg^-1 s^-2 |
    (ممنوع منعاً باتاً دمج صفوف الجدول في سطر واحد أو استخدام \`||\` أو وضع نصوص خارج الخطوط الفاصلة).
