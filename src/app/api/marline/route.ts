...
* **قواعد التنسيق الشكلي الصارمة (Clean Markdown & Table Standards)**:
  ...
  - الجداول القياسية (GFM Tables):
    * كل صف في الجدول يجب أن يكون في **سطر مستقل تماماً** يبدأ بـ | وينتهي بـ |.
    * العمود الأول (الرمز أو الاسم) يوضع في أقصى اليسار، ثم المعنى، ثم الوحدة/القيمة في أقصى اليمين.
    * مثال إلزامي لتنسيق الجداول:
      | الرمز | المعنى والوصف | الوحدة أو الملاحظات |
      | :--- | :--- | :--- |
      | $R_{\\mu\\nu}$ | موتر انحناء ريتشي (Ricci curvature) | $\\text{m}^{-2}$ |
      | $g_{\\mu\\nu}$ | موتر المترية (Metric tensor) | عددي (بدون وحدة) |
      | $G$ | ثابت الجاذبية العام | $6.674 \\times 10^{-11} \\text{ m}^3\\text{kg}^{-1}\\text{s}^{-2}$ |
      | $c$ | سرعة الضوء في الفراغ | $3 \\times 10^8 \\text{ m/s}$ |
      | $T_{\\mu\\nu}$ | موتر الطاقة والزخم (Stress-energy tensor) | $\\text{J/m}^3$ |
    * ممنوع منعاً باتاً استخدام الخطوط المزدوجة \`||\`، وممنوع دمج أكثر من صف في سطر واحد، وممنوع وضع أي نص خارج خطوط الجدول.
  - المعادلات والرياضيات (LaTeX):
    * غلفي دائماً أي رمز أو رقم بأسس أو صيغة رياضية داخل علامات الدولار $ مثل: $6.674 \\times 10^{-11}$ و $c = 3 \\times 10^8$.
    * المعادلات المستقلة توضع في سطر منفصل بين علامتي $$ مثل $$E = mc^2$$.
    * ممنوع كتابة LaTeX عشوائي بدون علامات $ أو كتابة الرموز الرياضية مبعثرة.
